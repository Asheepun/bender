...
void Engine_setPointerScale(float x, float y){
	Engine_pointer.scale = getVec2f(x, y);
}
	...
	while(!programShouldQuit){
		...
		while(XPending(dpy) > 0){
			...
			if(xev.type == MotionNotify){
				...
				Engine_pointer.pos.x = (int)((float)motionEvent_p->x / Engine_pointer.scale.x);
				Engine_pointer.pos.y = (int)((float)motionEvent_p->y / Engine_pointer.scale.y);
			}
