...
void Renderer2D_beginRectangle(Renderer2D_Renderer *renderer_p, float x, float y, float width, float height){
	...
	unsigned int vertexPositionAttributeLocation = glGetAttribLocation(renderer_p->currentShaderProgram.ID, "vertexPosition_attribute");
	unsigned int textureVertexAttributeLocation = glGetAttribLocation(renderer_p->currentShaderProgram.ID, "textureVertex_attribute");
